...
%		More importantly, the PAF can be generalised to activation functions other than NSP, such as the Rectified Linear Unit~(ReLU).%, and the parameters are independent to activation functions.
		...
		%  We extended the work of proposed activation function, NSP, to fit into training of layered up deep spiking neural networks~(SNNs).
...
%	The PAF version of the activation function (Eq.~\ref{equ:PAF}) will be linearly-scaled by the conversion parameter $S$ and the synaptic time constant $\tau_{\textit{\textrm{syn}}}$:
